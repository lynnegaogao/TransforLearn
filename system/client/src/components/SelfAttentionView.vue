  ...
  <div class="box">
    <div class="control-pannel">
      ...
      <div class="title-text">
        Multi-Head Self-Attention View 
      </div>
      <!--<button class="buttons" @click="onClean()">x</button>-->
      <el-tooltip content="Quit from Multi-Head Self-Attention View." placement="top" effect="dark">
        <el-button type="primary" :icon="CloseBold" plain circle @click="onClean()" />
      </el-tooltip>
    </div>
    <SelfAttentionAnimator
...
<script>
import SelfAttentionAnimator from "./SelfAttentionAnimator"

import { shallowRef } from "vue"
import { CloseBold } from '@element-plus/icons-vue'
export default {
  name:"SelfAttentionView",
  components:{
    SelfAttentionAnimator,
  },
  props:{
  ...
  methods:{
    ...
    onOpenHeadDetailView(headNum,attentionOuputTotalDataRange){
      let input = this.nodeData.input[0];
    ...
    // concatenateInColumns(rows,cols,step,array){
    //   let res = array2d(rows,cols,()=>0);
    //   let numElements = array.length;
    //   for(let r=0; r<rows; r++){
    //     for(let c=0; c<step; c++){
    //       for(let n=0; n<numElements; n++){
    //         res[r][c+n*step] = array[n][r][c];
    //       }
    //     }
    //   }
    //   return res;
