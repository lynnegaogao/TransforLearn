<template>
<div class="container">
  <svg id="selfAttentionSvg" height="100%"></svg>
</div>
</template>
...
export default {
  ...
  methods:{
    ...
    drawSvgLeft(){
      ...
      let svg = d3.select(this.$el).select('#selfAttentionSvg');
      ...
      let inputGridGroupY = 75;
      let inputData = this.nodeData.input[0];
      let inputConstraintGridCellWidth = getVisualizationSizeConstraint(inputData[0].length,'x',320,300,40);
      let inputConstraintGridCellHeight = getVisualizationSizeConstraint(inputData.length,'y',320,300,40);
      ...
      // 存储一些变量, 以备后续再次使用;
      this.inputGridGroupX = inputGridGroupX;
      this.inputGridGroupY = inputGridGroupY;
      this.inputGridData = inputGridData;
      this.inputTotalGridWidth = inputTotalGridWidth;
      this.inputTotalGridHeight = inputTotalGridHeight;
    },
    drawSvgRight(){
      ...
      let QKVTitleText = svg.append('text').attr("text-anchor","middle");
        QKVTitleText
          .attr('transform',`translate(${ HeadsGroupX + xSizeOfEachHead/2 },${HeadsGroupY/4})`)
          .attr('class','title-text')
          .attr('dy','.3em')
          .style('font-weight','bold')
          .style('font-size',fontSize/2)
          .text(`Query,Key,Value in all heads`);
      let shapeText = svg.append('text').attr("text-anchor","middle");
        shapeText
          ...
          .style('font-weight','bold')
          .style('font-size',fontSize/2)
          .text(`Size of Q, K, V in each head: (${this.nodeData.mat_QKV[0][0].length},${this.nodeData.mat_QKV[0][0][0].length})`);
    ...
    redraw(){
      // 移除之前所绘制的内容
      d3.select(this.$el).selectAll('#selfAttentionSvg > *').remove();
      d3.select(this.$el).select('#selfAttentionSvg').attr('width',1550);
      this.drawSvgLeft();
      this.drawSvgRight();
    },
    onHeadClick(headNum){
